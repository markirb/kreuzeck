...
class StrokeCollection {
    ...
    func squareBoundingRect(insetRatio: CGFloat = 0.4) -> CGRect {
        var frame: CGRect = .zero
        var first = true
        for stroke in strokes {
            for sample in stroke.samples {
                let rect = CGRect( origin: sample.location, size: .zero)
                if first {
                    first = false
                    frame = rect
                }
        ...
        let sizeDiff = frame.size.height - frame.size.width
        if sizeDiff > 0 {
            frame.size.width = frame.size.width + sizeDiff
            frame.origin.x = frame.origin.x - sizeDiff / 2.0
        }
        else {
            frame.size.height = frame.size.height - sizeDiff
            frame.origin.y = frame.origin.y + sizeDiff / 2.0
        }
        
        let inset = frame.size.width * insetRatio
