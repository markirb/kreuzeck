...
class StrokeCGView: UIView {
    var displayOptions = StrokeViewDisplayOptions.debug {
        didSet {
    ...
    var strokeCollectionBoundingView: UIView!
    ...
    override init(frame: CGRect) {
        ...
        view.layer.borderWidth = 0.5
        view.isUserInteractionEnabled = false
        strokeCollectionBoundingView = view
        self.addSubview(view)
    ...
    func draw(stroke: Stroke, in rect:CGRect, isActive active: Bool) {
        ...
        if displayOptions == .debug {
            ...
            if let strokeCollection = strokeCollection {
                strokeCollectionBoundingView.frame = strokeCollection.squareBoundingRect()
            }
